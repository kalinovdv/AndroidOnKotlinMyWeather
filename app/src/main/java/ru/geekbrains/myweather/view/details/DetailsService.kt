...
import ru.geekbrains.myweather.view.details.*
...
class DetailsService(name: String = "DetailsService") : IntentService(name){
    ...
    private fun loadWeather(lat: String, lon: String) {
        try {
            ...
            lateinit var urlConection: HttpsURLConnection
            try {
                urlConection = uri.openConnection() as HttpsURLConnection
                urlConection.apply {
                    requestMethod = REQUEST_GET
                    readTimeout = REQUEST_TIMEOUT
                    addRequestProperty(REQUEST_API_KEY, BuildConfig.WEATHER_API_KEY)
                }
                val weatherDTO: WeatherDTO = Gson().fromJson(getLines(BufferedReader(InputStreamReader(urlConection.inputStream))), WeatherDTO::class.java)
    ...
    private fun onSuccessResponse(temp: Int?, feelLike: Int?, condition: String?) {
        putLoadResult(DETAILS_RESPONSE_SUCCESS_EXTRA)
        broadcastIntent.putExtra(DETAILS_TEMP_EXTRA, temp)
        broadcastIntent.putExtra(DETAILS_FEELS_LIKE_EXTRA, feelLike)
